...
namespace CoffeeCard.Tests.Integration.Controllers.Account
{
    ...
    public class GetAccountTest : BaseIntegrationTest
    {
        ...
        {
            User user = UserBuilder.DefaultCustomer().Build();
            _ = await Context.Users.AddAsync(user);
            _ = await Context.SaveChangesAsync();
            SetDefaultAuthHeader(user);

            System.Net.Http.HttpResponseMessage response = await Client.GetAsync(GetAccountUrl);
            UserResponse account = await DeserializeResponseAsync<UserResponse>(response);
