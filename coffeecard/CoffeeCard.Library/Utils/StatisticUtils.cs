...
namespace CoffeeCard.Library.Utils
{
    public static class StatisticUtils
    {
        ...
        {
            double level = Math.Floor(1 + 1.0 / 10 * (Math.Sqrt(4 * userExperience + 25) - 5));
            return Convert.ToInt32(level);
        }

        public static int CalculateRequiredXpByLevel(int userExperience)
        {
            int lvl = CalculateLevelFromXp(userExperience);
            int xp = Convert.ToInt32(25 * Math.Pow(lvl, 2) + 25 * lvl);
            return xp;
        }
    }
}