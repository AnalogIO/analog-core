...
namespace CoffeeCard.Library.Services.v2
{
    public class WebhookService : IWebhookService
    {
        ...
        {
            IQueryable<WebhookConfiguration> webhooks = _context.WebhookConfigurations.Where(w => w.Status == WebhookStatus.Active);
        ...
        private async Task SyncWebhook(WebhookConfiguration webhook)
        {
            try
            {
                SingleWebhookResponse mobilePayWebhook = await _mobilePayWebhooksService.GetWebhook(webhook.Id);
        ...
        {
            SingleWebhookResponse mobilePayWebhook =
